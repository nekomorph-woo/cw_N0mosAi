# Why-First 引擎详解

**文档版本**: 1.0
**最后更新**: 2026-02-27
**作者**: Claude Opus 4.6

---

## 1. 概述

### 1.1 设计理念

Why-First 引擎是 nOmOsAi 系统的"深度思考"模块，其核心设计理念是 **"强制 AI 在动手前先思考为什么"**。

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Why-First 设计理念                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  传统 AI 编码模式（"想当然"模式）:                                    │
│  用户需求 → AI 立即开始写代码 → 发现问题 → 增量修补 → 越改越乱        │
│                                                                      │
│  nOmOsAi Why-First 模式（深度思考模式）:                              │
│  用户需求 → Why 问题生成 → 深度思考 → 知识沉淀 → Research → ...      │
│              ↑                              │                        │
│              └──────── 从 project-why.md 加载历史 ────┘              │
│                                                                      │
│  核心价值:                                                           │
│  - 避免幻觉: 强制 AI 思考而非假设                                     │
│  - 知识沉淀: 每次任务的"为什么"都沉淀到 project-why.md               │
│  - 避免重复错误: 历史教训自动加载到上下文                            │
│  - 提升质量: 前置思考减少后期返工                                    │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.2 核心功能

| 功能 | 说明 |
|------|------|
| **Why 问题生成** | 根据任务名称和描述生成 5-12 个针对性问题 |
| **知识库管理** | 维护 project-why.md 知识库 |
| **相似检测** | 检测新知识与已有条目的相似度，避免重复 |
| **知识增强** | 在现有条目基础上补充信息 |
| **知识合并** | 将相似条目合并，保持知识库整洁 |

### 1.3 与 Phase Gates 的关系

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Why-First 在 Phase Gates 中的位置                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  ┌───────────────┐    ┌───────────────┐    ┌───────────────┐        │
│  │  Why-First    │───►│   Research    │───►│     Plan      │        │
│  │   (内嵌阶段)   │    │   (调研阶段)   │    │   (计划阶段)   │        │
│  └───────────────┘    └───────────────┘    └───────────────┘        │
│         │                                                              │
│         │                                                              │
│         ▼                                                              │
│  ┌───────────────────────────────────────────────────────────────┐   │
│  │ Why-First 是 Research 阶段的第一个步骤:                         │   │
│  │ 1. 生成 Why 问题                                              │   │
│  │ 2. 从 project-why.md 加载相关历史                              │   │
│  │ 3. 在 research.md 的 "Why Questions" 部分回答问题              │   │
│  │ 4. 将重要决策沉淀到 project-why.md                             │   │
│  └───────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 2. 系统架构设计

### 2.1 架构定位

Why-First 引擎在 nOmOsAi 系统架构中的位置：

```
┌─────────────────────────────────────────────────────────────────────┐
│                      用户交互层                                      │
│  (Claude Code CLI / Task Viewer HTML 界面)                          │
└─────────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────────┐
│                      SKILL 编排层                                    │
│  (/nomos 主 SKILL + 子 SKILL)                                        │
│  ├── /nomos:start      → 启动完整流程（包含 Why-First）             │
│  └── /nomos:update-why → 更新 project-why.md                        │
└─────────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────────┐
│                    【Why-First 引擎在此层】                          │
│                      知识管理层                                      │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │ WhyFirstEngine                                                 │ │
│  │ ├── generate_why_questions()  → 生成 Why 问题                  │ │
│  │ ├── add_knowledge()           → 添加知识条目                   │ │
│  │ ├── search_knowledge()        → 搜索知识库                     │ │
│  │ ├── detect_similar_knowledge()→ 检测相似条目                   │ │
│  │ ├── enhance_knowledge()       → 增强现有条目                   │ │
│  │ └── suggest_merge()           → 建议合并条目                   │ │
│  └────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────────┐
│                      状态持久化层                                    │
│  ├── project-why.md          ← 【知识库文件】                       │
│  └── tasks/t{N}-{date}-{name}/                                      │
│       └── research.md         ← Why 问题回答                        │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 设计要求（从架构文档提取）

根据 `03_System_Architecture.md` 第 3.1 节，Why-First 引擎的设计要求：

| 需求 ID | 设计要求 | 来源 |
|---------|----------|------|
| FR-011 | 系统应支持 Why-First 阶段，通过苏格拉底式提问强制深度思考 | arch 3.1 |
| FR-022 | 系统应支持 project-why.md 智能维护 | arch 3.12 |
| FR-119 | 系统应支持 Knowledge Similarity Detector | arch 3.12.1 |
| FR-120 | 系统应支持 Knowledge Merge | arch 3.12.2 |

### 2.3 知识库结构设计

**project-why.md 文件结构**:

```markdown
---
project: nOmOsAi
created: 2026-02-26
version: 0.1.0
status: active
---

# Project Why - nOmOsAi 项目知识库

本文档记录项目的核心决策、经验教训和重要知识。

## 核心理念

### 为什么需要 nOmOsAi？

**时间**: 2026-02-26

[核心动机和价值的详细描述...]

## 架构决策

### 为什么选择 Markdown 作为状态存储？

**时间**: 2026-02-25

**原因**: Markdown 人类可读、Git 友好、跨会话持久化

**相关代码**: .claude/hooks/lib/

**最后更新**: 2026-02-26

**状态**: 仍有效

## 经验教训

### 2026-02-25: 必须走 Repository 层

**时间**: 2026-02-25

**错误**: 尝试直接调用 ORM 而非 Repository

**原因**: 未注意到事务管理依赖 Repository 层

**教训**: 所有数据访问必须通过 Repository 层

**补充** (2026-02-26):
增加了缓存层后，Repository 层的重要性更加突出

## 常见问题

### 如何处理跨模块依赖？

**时间**: 2026-02-25

[解决方案描述...]
```

---

## 3. 代码实现详解

### 3.1 文件结构

```
.claude/hooks/lib/
├── why_first_engine.py       # 核心 Why-First 引擎
└── ...

.claude/skills/nomos/prompts/
├── start.md                  # 任务启动流程（包含 Why-First）
└── update-why.md             # project-why.md 维护 prompt
```

### 3.2 WhyFirstEngine 类结构

**文件位置**: `/Volumes/Under_M2/a056cw/cw_nOmOsAi/.claude/hooks/lib/why_first_engine.py`

```python
class WhyFirstEngine:
    """Why-First 引擎"""

    def __init__(self, project_root: Optional[str] = None):
        """
        初始化 Why-First 引擎

        Args:
            project_root: 项目根目录
        """
        self.project_root = Path(project_root or os.getcwd())
        self.project_why_file = self.project_root / 'project-why.md'
```

**设计分析**:

- 单一职责：专注于 Why 问题生成和知识库管理
- 依赖最小化：仅依赖 Python 标准库
- 路径灵活性：支持自定义项目根目录

### 3.3 Why 问题生成

**代码位置**: `why_first_engine.py:27-46`

```python
def generate_why_questions(self, task_name: str, description: str) -> List[str]:
    """
    生成 Why 问题

    Args:
        task_name: 任务名称
        description: 任务描述

    Returns:
        Why 问题列表
    """
    questions = [
        f"为什么需要 {task_name}？",
        f"为什么现在做 {task_name}？",
        f"为什么选择这种方式实现 {task_name}？",
        f"为什么不用其他方案？",
        f"{task_name} 的核心价值是什么？"
    ]

    return questions
```

**问题维度分析**:

| 问题 | 维度 | 目的 |
|------|------|------|
| "为什么需要" | 核心动机 | 确认功能必要性 |
| "为什么现在做" | 时机选择 | 评估优先级 |
| "为什么选择这种方式" | 方案选择 | 验证技术决策 |
| "为什么不用其他方案" | 替代分析 | 避免遗漏更好方案 |
| "核心价值是什么" | 价值主张 | 明确成功标准 |

### 3.4 知识库管理

#### 3.4.1 添加知识条目

**代码位置**: `why_first_engine.py:48-113`

```python
def add_knowledge(self, category: str, title: str, content: str) -> bool:
    """
    添加知识到 project-why.md

    Args:
        category: 分类（核心理念/架构决策/经验教训/常见问题）
        title: 标题
        content: 内容

    Returns:
        是否成功
    """
    if not self.project_why_file.exists():
        return False

    # 读取现有内容
    with open(self.project_why_file, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    # 查找分类位置
    category_map = {
        '核心理念': '## 核心理念',
        '架构决策': '## 架构决策',
        '经验教训': '## 经验教训',
        '常见问题': '## 常见问题'
    }

    category_header = category_map.get(category)
    if not category_header:
        return False

    # 找到分类位置
    insert_index = -1
    for i, line in enumerate(lines):
        if line.strip() == category_header:
            # 找到下一个 ## 或文件末尾
            for j in range(i + 1, len(lines)):
                if lines[j].startswith('## '):
                    insert_index = j
                    break
            if insert_index == -1:
                insert_index = len(lines)
            break

    if insert_index == -1:
        return False

    # 插入新内容
    timestamp = datetime.now().strftime('%Y-%m-%d')
    new_content = [
        '\n',
        f'### {title}\n',
        '\n',
        f'**时间**: {timestamp}\n',
        '\n',
        f'{content}\n',
        '\n'
    ]

    lines[insert_index:insert_index] = new_content

    # 写回文件
    with open(self.project_why_file, 'w', encoding='utf-8') as f:
        f.writelines(lines)

    return True
```

**支持的分类**:

| 分类 | 用途 | 示例 |
|------|------|------|
| **核心理念** | 项目的核心价值观和设计理念 | "为什么需要 nOmOsAi" |
| **架构决策** | 重要的技术架构决策及其理由 | "为什么选择 Hooks 而非 Prompt" |
| **经验教训** | 失败案例和学到的教训 | "必须走 Repository 层" |
| **常见问题** | 常见问题及解决方案 | "如何处理跨模块依赖" |

#### 3.4.2 搜索知识

**代码位置**: `why_first_engine.py:115-145`

```python
def search_knowledge(self, keyword: str) -> List[Dict[str, str]]:
    """
    搜索知识库

    Args:
        keyword: 关键词

    Returns:
        匹配的知识条目列表
    """
    if not self.project_why_file.exists():
        return []

    with open(self.project_why_file, 'r', encoding='utf-8') as f:
        content = f.read()

    results = []
    sections = re.split(r'\n### ', content)

    for section in sections[1:]:  # 跳过第一个（YAML frontmatter 和标题）
        if keyword.lower() in section.lower():
            lines = section.split('\n', 1)
            title = lines[0].strip()
            body = lines[1] if len(lines) > 1 else ''

            results.append({
                'title': title,
                'content': body.strip()
            })

    return results
```

**搜索特点**:

- 大小写不敏感
- 基于 `###` 标题分节
- 返回标题和内容

#### 3.4.3 获取最近知识

**代码位置**: `why_first_engine.py:147-184`

```python
def get_recent_knowledge(self, limit: int = 5) -> List[Dict[str, str]]:
    """
    获取最近的知识条目

    Args:
        limit: 返回数量

    Returns:
        知识条目列表
    """
    if not self.project_why_file.exists():
        return []

    with open(self.project_why_file, 'r', encoding='utf-8') as f:
        content = f.read()

    results = []
    sections = re.split(r'\n### ', content)

    for section in sections[1:]:  # 跳过第一个
        lines = section.split('\n', 1)
        title = lines[0].strip()
        body = lines[1] if len(lines) > 1 else ''

        # 提取时间
        time_match = re.search(r'\*\*时间\*\*:\s*(\d{4}-\d{2}-\d{2})', body)
        timestamp = time_match.group(1) if time_match else '1970-01-01'

        results.append({
            'title': title,
            'content': body.strip(),
            'timestamp': timestamp
        })

    # 按时间排序
    results.sort(key=lambda x: x['timestamp'], reverse=True)

    return results[:limit]
```

### 3.5 知识相似度检测

#### 3.5.1 相似度计算

**代码位置**: `why_first_engine.py:248-259`

```python
def _calculate_similarity(self, text1: str, text2: str) -> float:
    """
    计算文本相似度

    Args:
        text1: 文本1
        text2: 文本2

    Returns:
        相似度 (0-1)
    """
    return SequenceMatcher(None, text1.lower(), text2.lower()).ratio()
```

**算法说明**:

- 使用 Python 标准库 `difflib.SequenceMatcher`
- 基于 longest common subsequence 算法
- 大小写不敏感

#### 3.5.2 相似条目检测

**代码位置**: `why_first_engine.py:208-246`

```python
def detect_similar_knowledge(self, new_content: str, threshold: float = 0.7) -> List[Dict[str, any]]:
    """
    检测相似的知识条目

    Args:
        new_content: 新内容
        threshold: 相似度阈值 (0-1)

    Returns:
        相似条目列表
    """
    if not self.project_why_file.exists():
        return []

    with open(self.project_why_file, 'r', encoding='utf-8') as f:
        content = f.read()

    similar_items = []
    sections = re.split(r'\n### ', content)

    for section in sections[1:]:
        lines = section.split('\n', 1)
        title = lines[0].strip()
        body = lines[1] if len(lines) > 1 else ''

        # 计算相似度
        similarity = self._calculate_similarity(new_content, body)

        if similarity >= threshold:
            similar_items.append({
                'title': title,
                'content': body.strip(),
                'similarity': similarity
            })

    # 按相似度排序
    similar_items.sort(key=lambda x: x['similarity'], reverse=True)

    return similar_items
```

**相似度阈值建议**:

| 相似度范围 | 建议操作 |
|-----------|---------|
| 0.8+ | 增强现有条目（添加补充信息） |
| 0.6-0.8 | 考虑合并条目 |
| < 0.6 | 创建新条目 |

### 3.6 知识合并与增强

#### 3.6.1 合并建议

**代码位置**: `why_first_engine.py:261-276`

```python
def suggest_merge(self, item1: Dict, item2: Dict) -> Dict[str, str]:
    """
    建议合并两个知识条目

    Args:
        item1: 条目1
        item2: 条目2

    Returns:
        合并建议
    """
    return {
        'merged_title': f"{item1['title']} & {item2['title']}",
        'merged_content': f"{item1['content']}\n\n---\n\n{item2['content']}",
        'reason': f"相似度: {item1.get('similarity', 0):.2%}"
    }
```

#### 3.6.2 知识增强

**代码位置**: `why_first_engine.py:278-312`

```python
def enhance_knowledge(self, title: str, additional_info: str) -> bool:
    """
    增强现有知识条目

    Args:
        title: 条目标题
        additional_info: 额外信息

    Returns:
        是否成功
    """
    if not self.project_why_file.exists():
        return False

    with open(self.project_why_file, 'r', encoding='utf-8') as f:
        content = f.read()

    # 查找条目
    pattern = rf'### {re.escape(title)}\n\n(.*?)(?=\n### |\Z)'
    match = re.search(pattern, content, re.DOTALL)

    if not match:
        return False

    # 在条目末尾添加信息
    old_content = match.group(0)
    new_content = old_content.rstrip() + f"\n\n**补充** ({datetime.now().strftime('%Y-%m-%d')}):\n{additional_info}\n"

    # 替换
    updated_content = content.replace(old_content, new_content)

    with open(self.project_why_file, 'w', encoding='utf-8') as f:
        f.write(updated_content)

    return True
```

**增强格式示例**:

```markdown
### 为什么选择 Markdown 作为状态存储？

**时间**: 2026-02-25

**原因**: Markdown 人类可读、Git 友好、跨会话持久化

**补充** (2026-02-26):
经过一周使用，发现 Markdown 的另一个优势：
支持在 IDE 中直接预览和编辑，无需额外工具
```

### 3.7 Why 问题验证

**代码位置**: `why_first_engine.py:186-206`

```python
def validate_why_answers(self, research_content: str) -> Dict[str, bool]:
    """
    验证 research.md 中是否回答了 Why 问题

    Args:
        research_content: research.md 内容

    Returns:
        验证结果字典
    """
    required_sections = [
        '为什么需要',
        '为什么选择',
        '为什么不'
    ]

    results = {}
    for section in required_sections:
        results[section] = section in research_content

    return results
```

**验证逻辑**:

- 检查 research.md 中是否包含必要的关键词
- 返回各关键词的存在状态

---

## 4. 设计 vs 实现对比

### 4.1 完成度分析

```
┌─────────────────────────────────────────────────────────────────────┐
│                     设计 vs 实现对比                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  核心功能                                                            │
│  ├── Why 问题生成        ✅ 已实现（固定 5 个问题）                  │
│  ├── 知识库添加           ✅ 已实现                                  │
│  ├── 知识库搜索           ✅ 已实现                                  │
│  ├── 相似度检测           ✅ 已实现                                  │
│  ├── 知识增强             ✅ 已实现                                  │
│  ├── 知识合并建议         ✅ 已实现                                  │
│  └── Why 答案验证         ✅ 已实现                                  │
│                                                                      │
│  设计文档要求                                                        │
│  ├── 针对性问题生成      ⚠️ 未实现（当前是固定问题）                 │
│  ├── 代码扫描集成        ❌ 未实现                                  │
│  ├── 历史 Why 加载       ⚠️ 部分实现（search_knowledge 存在）       │
│  └── 智能问题数量         ⚠️ 未实现（设计要求 5-12 个）              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.2 差异分析

#### 4.2.1 Why 问题生成

**设计要求** (`03_System_Architecture.md:764-781`):

```python
def generate_why_questions(requirement, project_why_md):
    # 1. 提取需求关键词
    keywords = extract_keywords(requirement)

    # 2. 扫描受影响范围(只看相关模块)
    affected_modules = scan_affected_modules(keywords)

    # 3. 从 project-why.md 加载相关历史
    historical_why = load_relevant_why(project_why_md, affected_modules)

    # 4. 生成 5-12 个针对性问题
    questions = []
    for module in affected_modules:
        questions.extend(generate_module_questions(module, historical_why))

    return questions[:12]  # 最多 12 个
```

**实际实现** (`why_first_engine.py:27-46`):

```python
def generate_why_questions(self, task_name: str, description: str) -> List[str]:
    questions = [
        f"为什么需要 {task_name}？",
        f"为什么现在做 {task_name}？",
        f"为什么选择这种方式实现 {task_name}？",
        f"为什么不用其他方案？",
        f"{task_name} 的核心价值是什么？"
    ]
    return questions
```

**差异说明**:

| 维度 | 设计要求 | 实际实现 | 差距 |
|------|----------|----------|------|
| **关键词提取** | 需要实现 | 未实现 | 缺少 NLP 能力 |
| **代码扫描** | 需要实现 | 未实现 | 缺少代码分析 |
| **历史加载** | 自动关联 | 需手动调用 | 缺少智能关联 |
| **问题数量** | 5-12 个 | 固定 5 个 | 缺少动态生成 |
| **针对性** | 基于模块 | 通用模板 | 缺少上下文感知 |

#### 4.2.2 建议增强实现

```python
def generate_why_questions_enhanced(self, task_name: str, description: str) -> List[str]:
    """
    增强版 Why 问题生成（建议实现）
    """
    # 1. 基础问题
    base_questions = [
        f"为什么需要 {task_name}？",
        f"为什么现在做 {task_name}？",
        f"{task_name} 的核心价值是什么？"
    ]

    # 2. 从描述提取关键词
    keywords = self._extract_keywords(description)

    # 3. 从 project-why.md 加载相关历史
    historical = []
    for keyword in keywords:
        historical.extend(self.search_knowledge(keyword))

    # 4. 基于历史生成针对性问题
    contextual_questions = []
    for item in historical[:3]:
        contextual_questions.append(
            f"与历史决策 '{item['title']}' 相比，{task_name} 有什么不同？"
        )

    # 5. 合并并去重
    all_questions = base_questions + contextual_questions
    return all_questions[:12]

def _extract_keywords(self, text: str) -> List[str]:
    """提取关键词（简单实现）"""
    # 停用词列表
    stop_words = {'的', '是', '在', '和', '了', '有', '不', '这', '我', '要'}

    # 简单分词（按空格和标点）
    words = re.findall(r'[\w\u4e00-\u9fff]+', text.lower())

    # 过滤停用词并返回
    return [w for w in words if w not in stop_words and len(w) > 1]
```

---

## 5. 使用流程图

### 5.1 Why-First 完整流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Why-First 完整流程                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  用户执行 /nomos:start <任务名>                                      │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 1: 初始化 Why-First 引擎                                   ││
│  │                                                                 ││
│  │ why_engine = WhyFirstEngine()                                  ││
│  │ questions = why_engine.generate_why_questions(task, desc)      ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 2: 从 project-why.md 加载相关历史                          ││
│  │                                                                 ││
│  │ historical = why_engine.search_knowledge(task_name)            ││
│  │ recent = why_engine.get_recent_knowledge(limit=5)              ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 3: 在 research.md 中回答 Why 问题                          ││
│  │                                                                 ││
│  │ ## 4. Why Questions                                             ││
│  │                                                                 ││
│  │ ### 为什么需要 {任务名}？                                       ││
│  │ [Agent 回答...]                                                 ││
│  │                                                                 ││
│  │ ### 为什么现在做 {任务名}？                                     ││
│  │ [Agent 回答...]                                                 ││
│  │ ...                                                             ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 4: 沉淀重要知识到 project-why.md                           ││
│  │                                                                 ││
│  │ # 检测是否已存在相似知识                                        ││
│  │ similar = why_engine.detect_similar_knowledge(new_knowledge)   ││
│  │                                                                 ││
│  │ if similar and similar[0]['similarity'] > 0.8:                 ││
│  │     # 增强现有条目                                              ││
│  │     why_engine.enhance_knowledge(                               ││
│  │         title=similar[0]['title'],                              ││
│  │         additional_info=new_knowledge                           ││
│  │     )                                                           ││
│  │ else:                                                           ││
│  │     # 添加新条目                                                ││
│  │     why_engine.add_knowledge(                                   ││
│  │         category="架构决策",                                     ││
│  │         title=f"{任务名} 的核心决策",                            ││
│  │         content=new_knowledge                                    ││
│  │     )                                                           ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 5: 进入 Research 阶段                                      ││
│  │                                                                 ││
│  │ 继续调研相关代码、识别 Protected Interfaces 等                   ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.2 知识库更新流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                      知识库更新流程                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Agent 准备更新 project-why.md                                       │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 1: 相似度检测                                              ││
│  │                                                                 ││
│  │ similar_items = detect_similar_knowledge(new_content, 0.7)     ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ├── 无相似条目                                               │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 2a: 直接添加新条目                                         ││
│  │                                                                 ││
│  │ add_knowledge(category, title, content)                         ││
│  │ ✅ 完成                                                         ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                      │
│         ├── 有相似条目                                               │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Step 2b: 根据相似度决定操作                                     ││
│  │                                                                 ││
│  │ ┌─────────────────────────────────────────────────────────────┐ ││
│  │ │ 相似度 > 0.8                                                │ ││
│  │ │ → 增强现有条目                                              │ ││
│  │ │ → enhance_knowledge(title, additional_info)                 │ ││
│  │ └─────────────────────────────────────────────────────────────┘ ││
│  │                                                                 ││
│  │ ┌─────────────────────────────────────────────────────────────┐ ││
│  │ │ 相似度 0.6-0.8                                               │ ││
│  │ │ → 建议合并                                                  │ ││
│  │ │ → suggest_merge(item1, item2)                               │ ││
│  │ │ → 用户确认后执行合并                                        │ ││
│  │ └─────────────────────────────────────────────────────────────┘ ││
│  │                                                                 ││
│  │ ┌─────────────────────────────────────────────────────────────┐ ││
│  │ │ 相似度 < 0.6                                                 │ ││
│  │ │ → 创建新条目                                                │ ││
│  │ │ → add_knowledge(category, title, content)                   │ ││
│  │ └─────────────────────────────────────────────────────────────┘ ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 6. 使用示例

### 6.1 启动 Why-First 流程

```python
import sys
sys.path.insert(0, '.claude/hooks')
from lib.why_first_engine import WhyFirstEngine

# 初始化引擎
why_engine = WhyFirstEngine()

# 生成 Why 问题
questions = why_engine.generate_why_questions("user-login", "实现用户登录认证功能")

print("Why Questions:")
for i, q in enumerate(questions, 1):
    print(f"  {i}. {q}")
```

**输出**:
```
Why Questions:
  1. 为什么需要 user-login？
  2. 为什么现在做 user-login？
  3. 为什么选择这种方式实现 user-login？
  4. 为什么不用其他方案？
  5. user-login 的核心价值是什么？
```

### 6.2 添加知识条目

```python
# 添加架构决策
success = why_engine.add_knowledge(
    category="架构决策",
    title="为什么选择 JWT 而非 Session",
    content="""
**原因**: 需要支持分布式部署和无状态认证

**备选方案**:
- Session: 需要共享存储，增加复杂度
- JWT: 无状态，易于扩展

**决策**: 选择 JWT

**相关代码**: src/auth/jwt_service.py
"""
)

if success:
    print("✅ 知识已添加到 project-why.md")
else:
    print("❌ 添加失败")
```

### 6.3 检测相似知识

```python
# 准备添加的新知识
new_knowledge = "用户认证应该使用 JWT Token 进行无状态验证..."

# 检测相似条目
similar_items = why_engine.detect_similar_knowledge(new_knowledge, threshold=0.7)

if similar_items:
    print(f"🔍 发现 {len(similar_items)} 个相似条目:\n")
    for item in similar_items:
        print(f"  - {item['title']} (相似度: {item['similarity']:.2%})")
        print(f"    {item['content'][:100]}...")
        print()

    # 根据相似度决定操作
    top_similar = similar_items[0]
    if top_similar['similarity'] > 0.8:
        print("💡 建议: 增强现有条目")
    elif top_similar['similarity'] > 0.6:
        print("💡 建议: 考虑合并")
    else:
        print("💡 建议: 创建新条目")
else:
    print("✅ 未发现相似条目，可以直接添加")
```

### 6.4 增强现有知识

```python
# 增强现有条目
success = why_engine.enhance_knowledge(
    title="为什么选择 JWT 而非 Session",
    additional_info="""
补充：考虑到移动端 API 调用场景，JWT 更适合无状态架构。
此外，JWT 支持自定义 Claims，可以在 Token 中携带用户角色信息。
"""
)

if success:
    print("✅ 知识已增强")
else:
    print("❌ 未找到该条目")
```

### 6.5 搜索知识

```python
# 搜索认证相关知识
results = why_engine.search_knowledge("认证")

print("搜索结果:")
for item in results:
    print(f"  - {item['title']}")
    print(f"    {item['content'][:80]}...")
    print()
```

### 6.6 获取最近知识

```python
# 获取最近 5 条知识
recent = why_engine.get_recent_knowledge(limit=5)

print("最近的知识条目:")
for item in recent:
    print(f"  - {item['title']} ({item['timestamp']})")
    print(f"    {item['content'][:80]}...")
    print()
```

---

## 7. 扩展指南

### 7.1 添加新的思考维度

**场景**: 想要添加"安全"维度的 Why 问题

**步骤 1**: 修改 `generate_why_questions` 方法

```python
def generate_why_questions(self, task_name: str, description: str,
                           dimensions: Optional[List[str]] = None) -> List[str]:
    """
    生成 Why 问题（支持自定义维度）

    Args:
        task_name: 任务名称
        description: 任务描述
        dimensions: 思考维度列表，默认为 ['core', 'timing', 'approach', 'value']
    """
    if dimensions is None:
        dimensions = ['core', 'timing', 'approach', 'value']

    dimension_questions = {
        'core': [
            f"为什么需要 {task_name}？",
            f"{task_name} 的核心价值是什么？"
        ],
        'timing': [
            f"为什么现在做 {task_name}？",
            f"不做 {task_name} 会有什么后果？"
        ],
        'approach': [
            f"为什么选择这种方式实现 {task_name}？",
            f"为什么不用其他方案？"
        ],
        'security': [
            f"{task_name} 有什么安全风险？",
            f"如何确保 {task_name} 的安全性？"
        ],
        'performance': [
            f"{task_name} 对性能有什么影响？",
            f"如何优化 {task_name} 的性能？"
        ],
        'value': [
            f"{task_name} 给用户带来什么价值？",
            f"如何衡量 {task_name} 的成功？"
        ]
    }

    questions = []
    for dim in dimensions:
        if dim in dimension_questions:
            questions.extend(dimension_questions[dim])

    return questions
```

**步骤 2**: 使用新维度

```python
# 添加安全维度
questions = why_engine.generate_why_questions(
    "user-login",
    "实现用户登录认证功能",
    dimensions=['core', 'security', 'performance']
)
```

### 7.2 添加新的知识分类

**场景**: 想要添加"性能优化"分类

**步骤 1**: 修改 `category_map`

```python
# 在 add_knowledge 方法中
category_map = {
    '核心理念': '## 核心理念',
    '架构决策': '## 架构决策',
    '经验教训': '## 经验教训',
    '常见问题': '## 常见问题',
    '性能优化': '## 性能优化',  # 新增
    '安全实践': '## 安全实践'   # 新增
}
```

**步骤 2**: 更新 project-why.md 模板

```markdown
## 性能优化

### 为什么使用 Redis 缓存？

**时间**: 2026-02-26

**原因**: 减少数据库查询压力，提升响应速度

**效果**: 接口响应时间从 200ms 降至 20ms

## 安全实践

### 为什么对所有用户输入进行转义？

**时间**: 2026-02-26

**原因**: 防止 XSS 攻击

**相关代码**: src/utils/sanitize.py
```

### 7.3 实现智能问题生成

**场景**: 基于代码扫描生成针对性问题

```python
def generate_contextual_questions(self, task_name: str, affected_files: List[str]) -> List[str]:
    """
    基于受影响文件生成针对性问题

    Args:
        task_name: 任务名称
        affected_files: 受影响的文件列表

    Returns:
        针对性 Why 问题列表
    """
    questions = []

    for file_path in affected_files:
        # 读取文件
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
        except:
            continue

        # 检测关键模式
        if 'TODO' in content:
            questions.append(f"文件 {file_path} 中有 TODO，是否需要在此任务中处理？")

        if 'deprecated' in content.lower():
            questions.append(f"文件 {file_path} 中有 deprecated 标记，如何处理？")

        if 'import' in content and 'auth' in content.lower():
            questions.append(f"{task_name} 是否会影响现有的认证逻辑？")

        # 从 project-why.md 加载相关历史
        related = self.search_knowledge(file_path)
        if related:
            questions.append(
                f"历史决策 '{related[0]['title']}' 与此任务相关，是否需要遵循？"
            )

    return questions[:5]  # 最多 5 个针对性问题
```

### 7.4 集成 AI 模型增强

**场景**: 使用 AI 模型生成更智能的问题

```python
def generate_ai_questions(self, task_name: str, description: str,
                          code_context: str) -> List[str]:
    """
    使用 AI 模型生成智能 Why 问题

    Args:
        task_name: 任务名称
        description: 任务描述
        code_context: 相关代码上下文

    Returns:
        AI 生成的 Why 问题列表
    """
    # 构建 prompt
    prompt = f"""
任务: {task_name}
描述: {description}

相关代码上下文:
```
{code_context[:2000]}
```

请生成 5-8 个深度思考问题，帮助开发者理解这个任务的本质。
问题应该涵盖:
1. 核心动机
2. 技术选择
3. 潜在风险
4. 依赖关系
5. 可扩展性

格式要求:
- 每行一个问题
- 问题以"为什么"或"如何"开头
- 避免过于宽泛的问题
"""

    # 调用 AI 模型（伪代码）
    # response = call_ai_model(prompt)
    # questions = parse_questions(response)

    # 这里返回模拟结果
    return [
        f"为什么 {task_name} 需要修改这些特定文件？",
        f"{task_name} 对现有系统有什么潜在影响？",
        f"如何确保 {task_name} 不会破坏现有功能？",
        f"{task_name} 是否需要考虑向后兼容？",
        f"{task_name} 的测试策略是什么？"
    ]
```

---

## 8. 与其他模块的协作

### 8.1 与 Phase Gates 的协作

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Why-First 与 Phase Gates 协作                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  /nomos:start                                                        │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Research 阶段                                                   ││
│  │                                                                 ││
│  │ ┌─────────────────────────────────────────────────────────────┐ ││
│  │ │ Why-First 子阶段                                             │ ││
│  │ │ - generate_why_questions()                                  │ ││
│  │ │ - 回答 Why 问题                                              │ ││
│  │ │ - add_knowledge() / enhance_knowledge()                     │ ││
│  │ └─────────────────────────────────────────────────────────────┘ ││
│  │                         │                                       ││
│  │                         ▼                                       ││
│  │ ┌─────────────────────────────────────────────────────────────┐ ││
│  │ │ 代码调研子阶段                                               │ ││
│  │ │ - 扫描相关代码                                               │ ││
│  │ │ - 识别 Protected Interfaces                                  │ ││
│  │ └─────────────────────────────────────────────────────────────┘ ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  PhaseManager.complete_phase("research", approved_by="human")       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.2 与 Revert Manager 的协作

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Why-First 与 Revert Manager 协作                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Revert 触发                                                        │
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Revert Manager 执行                                             ││
│  │ - git revert HEAD --no-edit                                     ││
│  │ - 记录到 code_review.md                                         ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Why-First 记录教训                                              ││
│  │                                                                 ││
│  │ why_engine.add_knowledge(                                       ││
│  │     category="经验教训",                                         ││
│  │     title=f"失败教训: {revert_reason}",                         ││
│  │     content=lesson_content                                       ││
│  │ )                                                               ││
│  └─────────────────────────────────────────────────────────────────┘│
│         │                                                            │
│         ▼                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ 回到 Plan 阶段                                                  ││
│  │ - 读取 project-why.md 中的教训                                  ││
│  │ - 重新设计方案                                                  ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.3 与 SessionStart Hook 的协作

**当前实现** (`nomos-session-start.sh`):

```bash
# SessionStart Hook: 显示当前任务提示 + 注入 nOmOsAi 规范
# ...
echo "║  流程规范: Why-First → Research → Plan → Execute → Review         ║"
# ...
```

**建议增强**: 在会话启动时加载相关 Why 知识

```bash
#!/bin/bash
# 增强版 SessionStart Hook

# ... 现有代码 ...

# 加载相关 Why 知识
if [ -f "$PROJECT_ROOT/project-why.md" ]; then
    if [ -f "$CURRENT_TASK_FILE" ]; then
        CURRENT_TASK=$(cat "$CURRENT_TASK_FILE")
        # 提取任务关键词
        TASK_NAME=$(basename "$CURRENT_TASK" | sed 's/t[0-9]*-[0-9-]*-//')

        # 显示相关 Why 知识（简化版）
        echo ""
        echo "📚 相关知识:"
        grep -A 3 "### .*$TASK_NAME" "$PROJECT_ROOT/project-why.md" 2>/dev/null | head -10
    fi
fi
```

---

## 9. 总结

### 9.1 核心价值

Why-First 引擎是 nOmOsAi 系统的"深度思考"模块，通过以下方式提升 AI 编码质量：

1. **强制深度思考**: 通过 Why 问题迫使 AI 在动手前思考
2. **知识沉淀**: 每次任务的决策和教训都沉淀到 project-why.md
3. **避免重复错误**: 历史教训自动加载到上下文
4. **智能维护**: 相似检测和知识合并保持知识库整洁

### 9.2 当前实现完成度

| 功能 | 完成度 | 说明 |
|------|--------|------|
| Why 问题生成 | 80% | 基础功能完成，缺少针对性生成 |
| 知识库管理 | 100% | 完整实现增删改查 |
| 相似度检测 | 100% | 使用 SequenceMatcher |
| 知识增强 | 100% | 支持追加补充信息 |
| 知识合并 | 90% | 建议功能完成，自动合并待实现 |
| Why 答案验证 | 100% | 关键词检测完成 |

### 9.3 后续优化方向

1. **智能问题生成**: 集成代码扫描和 AI 模型生成针对性问题
2. **历史关联**: 自动从 project-why.md 加载相关的历史决策
3. **知识图谱**: 构建知识条目之间的关联关系
4. **团队协作**: 支持多人共享和冲突合并

---

## 附录

### A. 文件路径索引

| 文件 | 路径 | 说明 |
|------|------|------|
| WhyFirstEngine | `.claude/hooks/lib/why_first_engine.py` | 核心 Why-First 引擎 |
| 启动流程 | `.claude/skills/nomos/prompts/start.md` | 任务启动流程（包含 Why-First） |
| 知识维护 | `.claude/skills/nomos/prompts/update-why.md` | project-why.md 维护 prompt |
| 知识库 | `project-why.md` | 项目知识库文件 |
| 架构文档 | `doc-arch/agent-nomos-flow/03_System_Architecture.md` | 系统架构设计 |

### B. 关键代码行号索引

| 功能 | 文件 | 行号 |
|------|------|------|
| WhyFirstEngine 类定义 | `why_first_engine.py` | 14-25 |
| Why 问题生成 | `why_first_engine.py` | 27-46 |
| 添加知识 | `why_first_engine.py` | 48-113 |
| 搜索知识 | `why_first_engine.py` | 115-145 |
| 获取最近知识 | `why_first_engine.py` | 147-184 |
| Why 答案验证 | `why_first_engine.py` | 186-206 |
| 相似度检测 | `why_first_engine.py` | 208-246 |
| 相似度计算 | `why_first_engine.py` | 248-259 |
| 合并建议 | `why_first_engine.py` | 261-276 |
| 知识增强 | `why_first_engine.py` | 278-312 |

### C. API 速查

| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| `generate_why_questions(task_name, description)` | 任务名, 描述 | List[str] | 生成 Why 问题 |
| `add_knowledge(category, title, content)` | 分类, 标题, 内容 | bool | 添加知识条目 |
| `search_knowledge(keyword)` | 关键词 | List[Dict] | 搜索知识库 |
| `get_recent_knowledge(limit)` | 数量限制 | List[Dict] | 获取最近条目 |
| `detect_similar_knowledge(content, threshold)` | 内容, 阈值 | List[Dict] | 检测相似条目 |
| `enhance_knowledge(title, additional_info)` | 标题, 补充信息 | bool | 增强现有条目 |
| `suggest_merge(item1, item2)` | 两个条目 | Dict | 建议合并 |
| `validate_why_answers(research_content)` | research 内容 | Dict[str, bool] | 验证 Why 答案 |

---

*本文档由 Claude Opus 4.6 生成*
*生成时间: 2026-02-27*
*来源: nOmOsAi 系统架构与代码分析*
