...
本文档详细讲解 nOmOsAi 项目的 Linter 系统设计与实现。
...
nOmOsAi Linter 系统的核心目标是实现 **"前置 Review"** -- 在代码写入之前就发现问题，而非传统的写完代码再 review。
...
│  nOmOsAi:  编码 → PreToolUse Linter → 阻塞/放行 → 写入       │
└──────────────────────────────────────────────────────────────┘
```
...
nOmOsAi Linter 系统通过三层规则体系，实现了从语法到业务的全面代码审查。核心特点包括:
