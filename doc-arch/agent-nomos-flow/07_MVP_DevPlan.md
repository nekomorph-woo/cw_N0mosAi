...
1. [阶段目标](#1-阶段目标)
2. [自举策略：延迟激活 Hooks](#2-自举策略延迟激活-hooks)
3. [Gate 0.1: 项目基础设施](#3-gate-01-项目基础设施)
4. [Gate 0.2: Task 状态管理器](#4-gate-02-task-状态管理器)
5. [Gate 0.3: AgentLinterEngine 核心](#5-gate-03-agentlinterengine-核心)
6. [Gate 0.4: 基础 Hooks](#6-gate-04-基础-hooks)
7. [Gate 0.5: 基础 SKILL](#7-gate-05-基础-skill)
8. [Gate 0.6: 文档模板](#8-gate-06-文档模板)
...
在开发 nOmOsAi 系统本身的过程中，存在一个**自举问题（Bootstrapping Problem）**：
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  问题: 如果 settings.json 已配置 Hooks，但 Hooks 脚本或依赖模块尚未完成，    │
│        会阻止 AI 写入代码吗？                                                │
│                                                                              │
│  风险点:                                                                     │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ PreToolUse Hook 依赖链:                                              │   │
│  │                                                                       │   │
│  │ settings.json → nomos-pretooluse.sh → lib/linter_engine.py           │   │
│  │                                     → lib/rules/*.py                  │   │
│  │                                     → 外部工具 (ruff, bandit)         │   │
│  │                                                                       │   │
│  │ ❌ 任何一个环节缺失 → Hook 失败 → 可能阻止代码写入                    │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  解决方案: 延迟激活 Hooks                                                    │
│                                                                              │
...
**核心原则**: 在 Gate 0.4 完成前，`settings.json` 中的 Hooks 配置保持为空，不激活任何 Hook。
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  Gate 0.1 ──► Gate 0.2 ──► Gate 0.3 ──► Gate 0.4 ──► Gate 0.5 ──► Gate 0.6 │
│     │            │            │            │            │            │      │
│     └────────────┴────────────┴────────────┼────────────┴────────────┘      │
│                                             │                                │
│                                             ▼                                │
...
│                                   └──────────────────┘                       │
│                                                                              │
│  激活前: settings.json 中 hooks 配置为空对象 {}                              │
│  激活后: 取消注释，启用完整的 Hooks 配置                                      │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 2.3 settings.json 配置模板

**开发期间（Gate 0.1 - Gate 0.4）**:
...
在激活 Hooks 之前，必须确认以下条件全部满足：

| 检查项 | 验证命令 | 期望结果 |
|--------|----------|----------|
| PreToolUse 脚本存在 | `test -f .claude/hooks/nomos-pretooluse.sh && echo OK` | OK |
| Stop 脚本存在 | `test -f .claude/hooks/nomos-stop.sh && echo OK` | OK |
| SessionStart 脚本存在 | `test -f .claude/hooks/nomos-session-start.sh && echo OK` | OK |
| linter_engine 可导入 | `python3 -c "from lib.linter_engine import AgentLinterEngine"` | 无报错 |
| Ruff 已安装 | `command -v ruff && echo OK` | OK |
| Bandit 已安装 | `command -v bandit && echo OK` | OK |
| 单元测试通过 | `pytest tests/` | 全部通过 |
...
│  └──────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                               │
│  Step 2: 修改 settings.json                                                 │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ 将 hooks: {} 替换为完整的 Hooks 配置                                  │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                               │
│  Step 3: 重启 Claude Code 会话                                              │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ 关闭当前会话，重新启动 Claude Code                                    │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                               │
│  Step 4: 验证 Hooks 已激活                                                  │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ SessionStart Hook 应显示任务提示                                      │   │
│  │ 尝试写入不合格代码，验证 PreToolUse Hook 拦截                         │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                               │
│  Step 5: 端到端测试                                                         │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ 执行完整的任务流程，验证所有 Hooks 正常工作                            │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 事项 | 说明 |
|------|------|
| ⚠️ 不要提前激活 | 在 Gate 0.4 完成前激活 Hooks 会导致开发受阻 |
| ⚠️ 外部工具依赖 | 确保 ruff 和 bandit 已通过 pip 安装 |
| ⚠️ 脚本权限 | Hook 脚本必须有执行权限 (`chmod +x`) |
| ⚠️ 会话重启 | 修改 settings.json 后需要重启 Claude Code 会话 |
...
| 配置文件 | `.claude/settings.json` | Hooks 配置入口 (开发期间为空) |
...
### 3.4 settings.json 配置（开发期间）

> ⚠️ **重要**: 在 Gate 0.4 完成前，Hooks 配置必须为空，避免自举问题。
> 详见 [2. 自举策略：延迟激活 Hooks](#2-自举策略延迟激活-hooks)

**Gate 0.1 - Gate 0.3 期间使用此配置**:
...
| 0.1.3 | 编写 `settings.json` (hooks: {}) | `.claude/settings.json` | JSON 格式合法，hooks 为空 |
...
- [ ] `settings.json` 中 `hooks` 为空对象 `{}`（延迟激活策略）
- [ ] `tasks/short-id-mapping.json` 为合法 JSON

---

## 4. Gate 0.2: Task 状态管理器
...
## 5. Gate 0.3: AgentLinterEngine 核心
...
> ⚠️ **重要**: 本 Gate 完成后，需要执行 [2. 自举策略：延迟激活 Hooks](#2-自举策略延迟激活-hooks) 中的"激活步骤"来启用 Hooks。
...
| 0.4.4 | 编写单元测试 | `tests/test_hooks.py` | 所有测试通过 |
| 0.4.5 | **激活 Hooks** | `settings.json` | 将 `hooks: {}` 替换为完整配置 |
| 0.4.6 | 重启 Claude Code 会话 | 手动操作 | 会话重启后 Hooks 生效 |
| 0.4.7 | 端到端测试 | 手动测试 | 完整流程可运行，Hooks 正常工作 |

> ⚠️ **步骤 0.4.5-0.4.7 是激活步骤**，详见 [2. 自举策略：延迟激活 Hooks](#2-自举策略延迟激活-hooks)
...
- [ ] 所有 Hook 脚本有正确的错误处理
- [ ] **Hooks 已在 settings.json 中激活**（延迟激活策略）
- [ ] 端到端测试通过
...
| 无 /nomos:clarify | 需求模糊时可能调研走偏 | Phase 1 |

### 11.3 P1 扩展规划: /nomos:clarify

**功能概述**: 轻量级需求澄清子命令，在 `/nomos:start` 之前帮助用户澄清模糊需求。

**开发优先级**: P1（增强用户体验，非 MVP 必需）

**交付物**:

| 交付物 | 路径 | 说明 |
|--------|------|------|
| clarify 提示词 | `.claude/skills/nomos/prompts/clarify.md` | 澄清流程提示词 |
| SKILL 更新 | `.claude/skills/nomos/SKILL.md` | 添加 clarify 子命令 |

**实施步骤**:

| 步骤 | 描述 | 验收条件 |
|------|------|----------|
| P1.1 | 编写 `clarify.md` 提示词 | 流程清晰，借鉴 doc-architect 结构化思维 |
| P1.2 | 更新 SKILL.md 添加子命令 | `/nomos:clarify` 可被识别 |
| P1.3 | 端到端测试 | 澄清报告在控制台正确显示 |

**Gate 完成条件**:

- [ ] `/nomos:clarify` 能启动澄清流程
- [ ] 澄清报告格式正确，包含关键词、痛点、约束
- [ ] 不生成任何文件，只在控制台输出
- [ ] 确认后正确提示用户使用 `/nomos:start`

**与 doc-architect 的关系**:

| 元素 | 复用程度 | 说明 |
|------|----------|------|
| 关键词提取 | 高 | 借鉴 Phase 2 提取逻辑 |
| 三维分类 | 中 | 简化为 3 类而非完整维度 |
| 文档映射 | 无 | clarify 不生成文档 |
| 验证流程 | 无 | clarify 是轻量级对话 |
...
| P1 clarify | FR-128, FR-129, FR-130 | US-130, US-131, US-132 |
...
| `.claude/skills/nomos/prompts/clarify.md` | Prompt | P1 |
