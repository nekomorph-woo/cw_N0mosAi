...
sequenceDiagram
    ...
    participant Task as Task 文件夹
    participant Hooks as Hooks 门控
    participant Git as Git 仓库

    User->>SKILL: /nomos:start 需求
    SKILL->>Git: 创建 feature 分支
    SKILL->>Task: 创建 task 文件夹
    SKILL->>Why: 加载相关 Why
    SKILL->>User: 生成 Why 问题
    User->>Task: 批注 research.md
    SKILL->>Why: 更新 project-why.md
    SKILL->>Task: 生成 plan.md
    User->>Task: 批注 plan.md
    SKILL->>Hooks: 请求写代码
    Hooks->>Task: 检查 Phase Gates
    Hooks->>SKILL: 放行/阻塞
    SKILL->>Git: commit + PR
```
